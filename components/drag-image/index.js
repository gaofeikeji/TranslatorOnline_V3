Component({
    ...
    ready() {
      this.createSelectorQuery()
        ...
        .boundingClientRect(({ top, left }) => {
          console.log('前top', top, 'left', left)
          // 计算 px 和rpx 的换算
          const { windowWidth } = wx.getSystemInfoSync();
          const rpx = windowWidth / 750;
          top = top / rpx;
          left = left / rpx;
          console.log('后top', top, 'left', left)
          this.setData({
  ...
  methods: {
    ...
    longPress(e) {
      ...
      console.log('longPress', 'pageX', pageX, 'pageY', pageY, 'previewSize', previewSize, 'top', top, 'left', left)
      // pageX 和 pageY都要乘上 rpx/px 的比例
      let _bili = 750 / wx.getSystemInfoSync().windowWidth;

      this.setData({
        currentIndex: index,
        // tranX: pageX - previewSize / 2 - left,
        // tranY: pageY - previewSize / 2 - top,
        tranX: pageX * _bili - previewSize / 2 - left,
        tranY: pageY * _bili - previewSize / 2 - top,
      });
      console.log('longPress', 'tranX', this.data.tranX, 'tranY', this.data.tranY)
    },
    ...
    touchMove(e) {
      ...
      const { previewSize, containerRes: { top, left } } = this.data;
      let _bili = 750 / wx.getSystemInfoSync().windowWidth;
      const tranX = pageX * _bili - previewSize / 2 - left;
      const tranY = pageY * _bili - previewSize / 2 - top;
      this.setData({
        tranX,
        tranY
      });
        ...
        wx.showActionSheet({
          ...
          success: async (res) => {
            if (res.tapIndex === 0) {
              ...
              this.setUploaPosition(dragImgList.length);
              console.log('dragImgList',dragImgList)
              this.setData({
                dragImgList,
              });
              this.updateEvent(dragImgList);
              ...
              const imgList = this.getDragImgList(res?.tempFiles?.map(({ path }) => path) || [], false);
              dragImgList = dragImgList.concat(imgList);
              this.setUploaPosition(dragImgList.length);
              this.setData({
                dragImgList,
